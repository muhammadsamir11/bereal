...
import { CURRENT_USER, FRIEND_USER, OFFICIAL_USER, NON_FRIEND_USER, MOCK_MEMORIES, PINNED_MEMORIES } from './constants';
...
const App: React.FC = () => {
  ...
  const onPanEnd = (event: any, info: PanInfo) => {
    ...
    const isRightSwipe = info.offset.x > SWIPE_THRESHOLD || info.velocity.x > VELOCITY_THRESHOLD;
    const isLeftSwipe = info.offset.x < -SWIPE_THRESHOLD || info.velocity.x < -VELOCITY_THRESHOLD;
  ...
  const showBottomNav = [View.HOME, View.FRIENDS, View.INBOX, View.CAMERA].includes(currentView);
      ...
      case View.OTHER_PROFILE:
        return (
          <ProfileView 
            ...
            pinnedMemories={
                viewingUser.id === CURRENT_USER.id ? PINNED_MEMORIES : 
                viewingUser.isFriend ? [...PINNED_MEMORIES].reverse() : 
                []
            }
            onEditProfile={() => {}}
            onBack={handleBack}
                ...
                <div className="p-4">
                    ...
                    {filteredFriends.length > 0 && (
                        <div className="flex flex-col gap-2 mb-4">
                            <p className="text-xs font-bold text-gray-500 uppercase tracking-wider px-2">My Friends</p>
                                ...
                                >
                                    <div className="flex items-center gap-3.5">
                                        <div className="relative">
                                            <img src={friend.avatarUrl} className="w-12 h-14 rounded-xl object-cover border border-white/20" alt={friend.displayName} />
                                        </div>
                                        <div className="flex flex-col justify-center">
                                            <p className="font-semibold text-[15px] leading-tight mb-0.5">{friend.displayName}</p>
                                            <p className="text-[13px] text-gray-500 font-medium">@{friend.username}</p>
                                        </div>
                                    </div>
                    ...
                    {filteredSuggestions.length > 0 && (
                        <div className="flex flex-col gap-2">
                            <p className="text-xs font-bold text-gray-500 uppercase tracking-wider px-2">RECOMMENDED FRIENDS</p>
                            ...
                            {filteredSuggestions.map(user => (
                                ...
                                >
                                    <div className="flex items-center gap-3.5">
                                        <div className="relative">
                                            <img src={user.avatarUrl} className="w-12 h-14 rounded-xl object-cover border border-white/20" alt={user.displayName} />
                                        </div>
                                        <div className="flex flex-col justify-center">
                                            <div className="flex items-center gap-1">
                                                <p className="font-semibold text-[15px] leading-tight mb-0.5">{user.displayName}</p>
                                                {user.isVerified && (
                                                    <BadgeCheck size={14} className="text-blue-500" fill="currentColor" stroke="black" />
                                                )}
                                            </div>
                                            <p className="text-[13px] text-gray-500 font-medium">@{user.username}</p>
                                        </div>
                                    </div>
                                    <Button variant="primary" size="sm" className="h-8 px-3 text-xs font-bold border-0 gap-1">
...
<rect width="84" height="18" fill="white"/>
    ...
    <div 
        className="relative z-10 bg-black flex flex-col h-screen max-h-screen sm:max-h-[650px] w-full max-w-md sm:w-[360px] mx-auto border-0 border-white/10 sm:border rounded-none sm:rounded-3xl overflow-hidden shadow-2xl"
    >
        ...
        {showBottomNav && (
             <BottomNav currentView={currentView} onNavigate={(v) => handleNav(v, 0)} />
        )}
    </div>
    </div>
  );
