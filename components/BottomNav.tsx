...
const BottomNav: React.FC<BottomNavProps> = ({ currentView, onNavigate }) => {
  const isActive = (view: View) => 
    currentView === view || (view === View.PROFILE && currentView === View.EDIT_PROFILE);
      ...
      <div className="bg-gradient-to-br p-4 from-[#2C2C2E]/60 to-black rounded-t-3xl flex justify-between items-end max-w-md mx-auto">
        ...
        >
          <motion.div variants={navItemVariants}>
            <Users 
              size={24} 
              className={`transition-colors duration-300 ${isActive(View.FRIENDS) ? 'fill-white' : 'fill-transparent'}`}
        ...
        >
           <motion.div 
             ...
             className={`w-[24px] h-[28px] rounded-lg overflow-hidden border transition-colors duration-300 ${isActive(View.PROFILE) ? 'border-white' : 'border-gray-600'}`}
           >
              <img src="https://images.unsplash.com/photo-1494790108377-be9c29b29330?auto=format&fit=crop&w=1000&q=80" alt="Profile" className="w-full h-full object-cover rounded-md" />
           </motion.div>
           <motion.span 
             variants={labelVariants}
             className="text-[11px] font-medium"
