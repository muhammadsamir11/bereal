...
import { Share2, MoreHorizontal, X, Users, ArrowLeft, Lock, Check, Instagram, Ghost, BadgeCheck, Link as LinkIcon, Flame, Smile } from 'lucide-react';
import Button from './Button';
import MemoriesCalendar from './MemoriesCalendar';
import { motion, animate, useMotionValue, AnimatePresence, useScroll, useTransform, useAnimation } from 'framer-motion';
...
const Confetti = () => {
  const particles = useMemo(() => Array.from({ length: 50 }).map((_, i) => ({
    ...
    color: ['#FFFFFF', '#FFD700', '#FF453A', '#32D74B', '#0A84FF'][Math.floor(Math.random() * 5)],
...
const ProfileView: React.FC<ProfileViewProps> = ({ 
  ...
  const coverBlur = useTransform(scrollY, [-100, 0, 300], ["blur(0px)", "blur(0px)", "blur(12px)"]); // Blur on scroll down
  ...
  return (
    <div ref={containerRef} className="bg-black text-white h-full overflow-y-auto overflow-x-hidden font-sans relative scrollbar-hide">
      ...
      <div className="relative w-full h-[350px] bg-[#1C1C1E] overflow-hidden z-0">
          ...
          <div className="absolute top-0 left-0 right-0 p-4 flex justify-between items-start z-30">
              <button 
                onClick={onBack}
                className="w-10 h-10 flex items-center justify-center text-white hover:bg-white/20 rounded-xl transition-colors bg-black/40"
              >
              ...
              {isPrivate ? (
                 ...
                 <div className="flex flex-col items-center justify-center pt-10 text-center animate-in fade-in duration-500 delay-150">
                    <div className="relative mb-4">
                        ...
                        />
                        <Lock size={32} strokeWidth={1.5} className="relative z-10 text-white/80" />
                    </div>
                    <h3 className="text-lg font-medium text-white/80">This account is private</h3>
                 </div>
              ) : (
                <>
                            ...
                            <AnimatePresence mode="popLayout" initial={false}>
                                {displayPins.map((pin) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            <div className={`w-[110px] aspect-[9/15] rounded-xl overflow-hidden relative bg-gray-900 border border-white/5 ${isEditingPins ? 'ring-2 ring-white/30' : ''}`}>
                                                <img src={pin.imageUrl} className="w-full h-full object-cover opacity-100" alt="Pin" />
                                                ...
                                                {pin.selfieUrl && (
                                                    <div className="absolute top-2 left-2 w-[30px] h-[40px] rounded-md overflow-hidden border border-white z-10 shadow-sm -rotate-[2deg] bg-black">
                                                        <img src={pin.selfieUrl} className="w-full h-full object-cover" alt="Selfie" />
                                                    </div>
                                                )}
                                                ...
                                                {isEditingPins && !isOfficial && (
                                                    <button
                                                        onClick={(e) => { e.stopPropagation(); setPinToRemove(pin.id); }}
                  ...
                  {!isCurrentUser && !isPrivate && (
                    <div className="mt-0">
                       ...
                       <div className="grid grid-cols-3 gap-2">
                          {memories.map((memory) => (
                             <div key={memory.id} className="relative aspect-[3/4] bg-gray-900 overflow-hidden group rounded-xl">
                                <img src={memory.imageUrl} alt="Memory" className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" />
                                ...
                                {memory.selfieUrl && (
                                    <div className="absolute top-2 left-2 w-[28%] aspect-[3/4] rounded-md overflow-hidden border border-white z-10 shadow-md -rotate-3 bg-black">
                                        <img src={memory.selfieUrl} className="w-full h-full object-cover" alt="Selfie" />
                                    </div>
                                )}
                                ...
                                <div className="absolute bottom-2 left-2 flex items-center gap-1 text-white/90 z-20">
                                   ...
                                   <span className="text-xs font-bold">{memory.realMojiCount || 0}</span>
                                </div>
                             </div>
                          ))}
                       </div>
           ...
           <div className="absolute inset-0 z-[100] flex items-end justify-center sm:items-center px-0 sm:px-4">
               ...
               >
                  ...
                  <div className="pt-2 px-6 flex flex-col items-center w-full">
                      <h3 className="text-white font-bold text-2xl mb-8 mt-4 tracking-tight">Share Profile</h3>
                              ...
                              <div className="w-full aspect-square relative mb-4 bg-white rounded-2xl overflow-hidden">
                                  ...
                                  <div className="absolute inset-0 flex items-center justify-center">
                                      <div className="w-[72px] h-[72px] bg-white rounded-full p-1.5 shadow-2xl ring-4 ring-white">
                                          <img src={user.avatarUrl} className="w-full h-full object-cover rounded-full bg-gray-100" alt="Avatar" />
                                      </div>
                                  </div>
                              </div>
                              <div className="text-center space-y-1">
                                  <p className="text-black font-black text-2xl tracking-tight leading-none">@{user.username}</p>
                                  <p className="text-gray-400 text-[10px] font-extrabold uppercase tracking-[0.2em]">BEREAL.</p>
                              </div>
                          </div>
                      </motion.div>
                      ...
                      <div className="grid grid-cols-4 gap-3 w-full px-2">
                          ...
                          <button className="flex flex-col items-center gap-2 group cursor-pointer">
                             ...
                             <span className="text-[11px] font-semibold text-white/90 tracking-wide">Stories</span>
                          </button>
                          ...
                          <button className="flex flex-col items-center gap-2 group cursor-pointer">
                             ...
                             <span className="text-[11px] font-semibold text-white/90 tracking-wide">Snapchat</span>
                          </button>
                          ...
                          >
                             ...
                             <span className="text-[11px] font-semibold text-white/90 tracking-wide">More</span>
                          </button>
                           ...
                           <button 
                             ...
                             onClick={handleCopyLink}
                            >
                             <div className={`w-16 h-16 rounded-[22px] flex items-center justify-center shadow-lg group-hover:scale-105 group-active:scale-95 transition-all duration-300 border border-white/10 ${isCopied ? 'bg-white text-black' : 'bg-white/10 backdrop-blur-md hover:bg-white/20 text-white'}`}>
                             ...
                             <span className="text-[11px] font-semibold text-white/90 tracking-wide">{isCopied ? 'Copied' : 'Link'}</span>
                          </button>
                      </div>
                  </div>
               </motion.div>
      ...
      <AnimatePresence>
          {pinToRemove && (
              <div className="absolute inset-0 z-[100] flex items-center justify-center px-6">
                   ...
                   >
                      <h3 className="text-xl font-bold text-white text-center mb-2">Unpin Memory?</h3>
                      <p className="text-white/60 text-center text-[15px] leading-relaxed mb-6">
                          This will remove this memory from your profile highlights. You can always add it back later.
                      </p>
